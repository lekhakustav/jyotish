...
| Home Baje starters | Heart, briefcase, mind-and-body icons; three two-line prompts | Same semantic vector icons, row height, separators, type, chevrons, and prompts | Matched |
...
| Family header/tree | `Parivar`, add action, self node, connector lines, relation nodes | Same title, action, node hierarchy, connectors, and localized relation labels | Matched |
