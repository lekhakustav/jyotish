...
substitute a generic landing-screen scroll. Keep one platform per creative and preserve enough pre/post-roll
...
| --- | --- | ---: | --- | --- |
| `SH-FAM-01` | Launch with `-demoSeed`; open My Kundli & QR and pause on Sita, Aarav, Priya with relationship labels. | 6s | More than one saved profile and the relationship labels. | Crop away birth details. `fam001` |
| `SH-FAM-02` | Tap Aarav from My Kundli & QR and pause on member detail; return or invoke the member-specific action used by the current build. | 8s | The selected person is visibly Aarav / son and the action belongs to that profile. | Do not show DOB/time/place. `fam001` |
...
| `SH-LANG-02` | In Nepali mode, move My Kundli & QR → Religious/Patro and back at natural speed. | 12s | Authentic Devanagari across two shipped surfaces. | Fluent review required; do not claim Hindi. `nep001` |
| `SH-PAT-01` | From Religious, tap the BS date/tithi block to open Patro. | 8s | Real Religious-tab date context and real Patro destination. | Do not imply a notification. `pat001` |
...
| `SH-KUN-01` | My Kundli & QR → Sita or Aarav → member detail; hold the North-Indian Kundali chart and calculated labels. | 10s | Chart plus Lagna/Rashi/Nakshatra context. | Never overlay altered placements. `kun001` |
...
| `SH-DSH-01` | Religious feature grid → Dashas & Life Phase → explanation sheet → continue. | 10s | Real shipped entry and explanation. | No future-alert or notification imagery. `dsh001` |
...
| `SH-MUH-01` | Religious feature grid → Muhurat Finder → choose a real purpose such as home entry. | 12s | Purpose selection and real transition to report/question. | The purpose must match the generated scene. `muh001` |
...
| `SH-MAT-01` | Religious → Kundli Matching → choose the synthetic second profile. | 12s | Two-person selection with real relationship context. | Hide DOB/time/place. `mat001` |
...
| `SH-QR-01` | Open Share My Kundli and its sensitive-data explanation using the synthetic Sita profile. | 10s | Real trust explanation and feature entry. | Mask the entire QR region in every exported frame. `dia001` |
...
| `SH-QR-03` | Complete import and show the synthetic profile in My Kundli & QR; test duplicate rejection separately for QA. | 10s | Saved result without automatic-sync implication. | Do not claim account transfer or syncing. `dia001` |
| `SH-REL-01` | From Religious Relationships or another social feature, open the person chooser and select Aarav. | 10s | Selected son relationship and correct destination. | Complete birth profile required. `rel001` |
...
| `SH-PAN-01` | Religious → Today's Panchang/feature sheet → continue to report or Patro day detail. | 10s | Real current-day entry and selected location. | Capture date/place in metadata. `pan001` |
