...
end_card_clip="$repo_root/marketing/media/launch-001/endcard/med_20260719_endcard01__cmp_20260716_launch__store-marks-dynamic-bg.mp4"
output_video="${1:-$repo_root/marketing/media/launch-001/prototypes/cou001/med_20260718_cou001v6__cmp_20260716_launch__reusable-endcard.mp4}"

mkdir -p "$(dirname "$output_video")"

# Reuses the standalone endcard01 clip (marketing/media/launch-001/endcard/) so this creative
# and any future one can share the same rendered end card instead of baking a new one per ad.
ffmpeg -hide_banner -loglevel error -y \
  ...
  -filter_complex "
    ...
    [3:v]trim=duration=2.5,setpts=PTS-STARTPTS,scale=720:1280,setsar=1[card];
