# Marketing charter

## Mission

Make Jyotish Baje the trusted family-first Jyotish companion for Nepali households and the
people who care for them, using culturally specific short-form storytelling backed by the
app's real calculation, family, Patro, and bilingual guidance capabilities.

## What we are building

An end-to-end creative learning loop:

```text
research -> audience tension -> concept -> Veo scene -> app proof -> edit -> publication
         -> platform response -> activated household -> retention -> decision -> next test
```

The first creative format is a vertical Reel/TikTok assembled from an 8-second Veo scene
(12 seconds only when the story requires it), authentic app usage capture, AI voice, captions,
sound, and a single call to action. The generated scene earns attention; the app footage proves
the promise.

## Product and installer roles

The documented product user is a Nepali parent or grandparent aged 40–80. The person most
reachable on Instagram or TikTok may instead be an adult child, partner, or diaspora family
member who installs it for themselves or introduces it to the household. These roles must be
measured separately rather than collapsed into one imaginary “astrology user.”

## Measurement hierarchy

The app is currently free, with no subscription or in-app purchase. Therefore revenue, ROAS,
and profit are not valid launch optimization metrics.

1. **Creative diagnostic:** hold, completion, shares, saves, qualified comments.
2. **Acquisition:** outbound clicks and attributable installs where available.
3. **Activation:** birth profile saved plus first completed personalized answer or report
   within 24 hours of first open.
4. **Retention:** activated household returns for personalized guidance in D1, D7, and D30.
5. **North star:** weekly guided households receiving a completed personalized answer or
   report.

Until install attribution is implemented and legally reviewed, platform-to-app conclusions
must be described as directional. UTMs measure clicks but do not reliably survive every app
store install path.

## Operating principles

- Truth beats spectacle. Every promise must be visible in the shipped app.
- Family is the moat. Generic zodiac content is a discovery surface, not the strategy.
- Emotion earns the next second; app proof earns the install.
- Change one dominant factor per controlled test.
- Preserve failed ads and null results. They are training data for the next decision.
- No honest operator guarantees virality. We maximize the probability of useful attention,
  activation, and retained use through evidence and iteration.

## Initial 90-day outcome

Build a reliable baseline across at least four audience roles, six hook families, two
languages, and multiple proof features; verify the data pipeline with an A/A test; then run
pre-registered creative tests until the team can predict which concepts improve qualified
watching and activated-household cost relative to its own baseline.
