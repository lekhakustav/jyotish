...
function dimensions(kind) {
  return kind === "tiktok"
    ? { width: 1080, height: 1920, headerY: 82, ruleY: 120, pageY: 82, logoY: 190, titleY: 520, titleSize: 72, titleGap: 100, omY: 830, cardY: 980, cardH: 480, dayY: 1110, mantraY: 1260, mantraSize: 66, mantraGap: 92, footerY: 1795 }
    : { width: 1080, height: 1350, headerY: 75, ruleY: 110, pageY: 75, logoY: 155, titleY: 365, titleSize: 82, titleGap: 105, omY: 600, cardY: 760, cardH: 360, dayY: 865, mantraY: 1010, mantraSize: 55, mantraGap: 78, footerY: 1270 };
}
...
function shell(inner, page, kind) {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${d.width}" height="${d.height}" viewBox="0 0 ${d.width} ${d.height}">
    ...
    <text x="1008" y="${d.pageY}" text-anchor="end" class="sans" font-size="20" fill="${palette.muted}">${String(page).padStart(2, "0")} / 08</text>
...
function cover(kind) {
  ...
  return shell(`
    <image href="${logo}" x="72" y="${d.logoY}" width="92" height="92"/>
...
function daySlide(day, index, kind) {
  ...
  const firstMantraY = lines.length === 1 ? d.mantraY + 45 : d.mantraY;
  const mantra = lines.map((line, lineIndex) => `<tspan x="${center}" dy="${lineIndex === 0 ? 0 : lineGap}">${escapeXml(line)}</tspan>`).join("");
  return shell(`
    <rect x="72" y="${d.cardY}" width="936" height="${d.cardH}" rx="28" fill="${color}" opacity=".10"/>
    <rect x="72" y="${d.cardY}" width="14" height="${d.cardH}" rx="7" fill="${color}"/>
    <text x="${center}" y="${d.dayY}" text-anchor="middle" class="serif" font-size="${kind === "tiktok" ? 108 : 94}" font-weight="700" fill="${color}">${escapeXml(day.day)}</text>
    <line x1="270" y1="${d.dayY + 56}" x2="810" y2="${d.dayY + 56}" stroke="${color}" stroke-opacity=".35"/>
    <text x="${center}" y="${firstMantraY}" text-anchor="middle" class="serif" font-size="${kind === "tiktok" ? 70 : d.mantraSize}" fill="${palette.ink}">${mantra}</text>
    ${progress(index + 1, kind)}
  `, index + 2, kind);
}
...
async function renderKind(kind) {
  ...
  const slides = [cover(kind), ...content.days.map((day, index) => daySlide(day, index, kind))];
  const pngPaths = [];
